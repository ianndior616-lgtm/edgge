/**
 * МИГРАЦИИ СХЕМЫ, ЗАШИТЫЕ В БАНДЛ.
 *
 * SQL сгенерирован drizzle-kit (см. drizzle/0000_initial.sql) и продублирован
 * здесь строкой, чтобы application-side runner (/api/admin/migrate) работал на
 * serverless Vercel, где нет доступа к файлам репозитория в рантайме.
 *
 * КАК ДОБАВИТЬ НОВУЮ МИГРАЦИЮ:
 *   1. измените src/db/schema.ts;
 *   2. npx drizzle-kit generate --name <change>  (сгенерирует drizzle/NNNN_*.sql);
 *   3. добавьте содержимое нового .sql в массив MIGRATIONS ниже (отдельным
 *      элементом с именем файла без расширения).
 *
 * Каждая миграция выполняется в транзакции и помечается в таблице _migrations,
 * поэтому повторный вызов безопасен (идемпотентен).
 */
...
export interface Migration {
  ...
  name: string;
  /** SQL миграции: допускается несколько стейтментов через «;». */
  sql: string;
}
